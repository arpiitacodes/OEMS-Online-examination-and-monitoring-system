...
waitress-serve --host=127.0.0.1 --port=5000 oems:app
```

(For `oems:app`, ensure you run this from `backend\` so Python imports `oems.py`.)
...
| `ModuleNotFoundError: No module named 'flask'` | venv not activated | Activate venv before running `oems.py`. |
| `Address already in use` / port 5000 busy | Another process on 5000 | Stop the other process or change the port in `oems.py`. |
...
A: Yes — edit `app.run(host='127.0.0.1', port=5000, ...)` at the bottom of `oems.py`, and update the Electron `baseUrl` / Nginx `proxy_pass` accordingly.
